package com.openclassrooms.starterjwt.TestUnitaire.ControllerTest;
...
class SessionControllerTest {
    ...
    void testUpdate_Success() {
        ...
        when(sessionMapper.toEntity(sessionDto)).thenReturn(sessionToUpdate);  // Transforme le DTO en entité
        when(sessionService.update(1L, sessionToUpdate)).thenReturn(sessionToUpdate);  // Assure que le service renvoie la session mise à jour
        when(sessionMapper.toDto(sessionToUpdate)).thenReturn(sessionDto);  // Map la session mise à jour en DTO

        // Act
        ResponseEntity<?> response = sessionController.update("1", sessionDto);  // Utilisation du DTO dans le contrôleur
    ...
    void testFindById_BadRequest() {
        // Act
        ResponseEntity<?> response = sessionController.findById("abc");  // Passons une chaîne non numérique
